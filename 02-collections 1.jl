...
# ╔═╡ d5f9cca6-5a89-492a-b960-6b969ef56792
function integrate1(a,b,n)
	x= [a+(b-a)/n*i for i=1:n];
	f = [x[i]*sin(x[i]) for i=1:n];
	return f
end


# ╔═╡ 239343a9-7bc7-46d3-a866-f7ee749f77e8
function integrate2(a,b,n)
	x= [a+(b-a)/n*i for i=1:n];
	f = [sqrt(1-x[i]^2) for i=1:n];
	return f
end
...
begin 
	f1 = integrate1(0, 2π, 100)
    @show integral1=riemannsum(f1, 0, 2π, n=100)
	f2 = integrate2(0, 1, 1000)
	@show integral2=riemannsum(f2, 0, 1, n=100)
end


# ╔═╡ ba396c9a-3aea-40ba-a1d4-cad0ca8dd6f1
integral1

# ╔═╡ fc5bf9a5-f1c1-416b-af60-615854f6e237
integral2
...
# ╔═╡ fb82659f-a359-42d1-88d3-a84b69d1b21f
PM = [0 1 ; 2 3]

# ╔═╡ b954bed5-6a0c-43ad-b9a4-f3ceba133c9b
mydet(PM)
...
# ╔═╡ 8bf6e649-1d03-4bed-9069-c9dd3abb7936
estimatepi(1000000)

# ╔═╡ c29da370-bce2-452f-8ea6-1a66c5c013b1
begin
estimatepi2(n) = 4count(x->x≤1.0, sum(rand(n,2).^2, dims=2)) /n
end

# ╔═╡ 7638246e-5723-4e71-b6c1-08b78b26a38f
estimatepi2(1000)
