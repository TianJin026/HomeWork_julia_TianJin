...
student = (name = "Tian Jin", email = "Tian.Jin@UGent.be");
...
function isdisjoint(data)
	distant_list = Vector{Float64}()
	for i_overlap in (1:length(data))
		distant_list_i = Vector{Float64}()
		for j_overlap in (i_overlap+1:length(data))
			dist = euclidean([data[i_overlap].x, data[i_overlap].y], [data[j_overlap].x, data[j_overlap].y])
			if dist <= data[i_overlap].R + data[j_overlap].R
				append!(distant_list_i, dist)
			end
		end
		if length(distant_list_i) == 0
			append!(distant_list, 0)
		else
			append!(distant_list, 1)
		end
	end
	println("the number of shapes overlaped:", count(i->(i==1), distant_list))
	return distant_list
end
...
# ╔═╡ 001b97d9-5b34-4f7a-b5b5-e74cf3bde270
begin
xr=rand(n_points)*100;
yr=rand(n_points)*100;
	length_points=length(xr);
end

# ╔═╡ e1a4741f-415d-4a61-8ff5-a8454607d437
function in(xr,yr,length_points,shapes)
	N=0;
	
	for i=1:length_points
		distance = getfield.(shapes,3).^2 .-(getfield.(shapes,1).-xr[i]).^2 .-(getfield.(shapes,2).-yr[i]).^2;
		 positive = count(>(0),distance);
		if positive>0
		N=N+1;
		end
	end
...
# ╔═╡ 5355001e-9da7-41aa-a074-1c421c11868f
area(shapes)

# ╔═╡ 88ae68d6-1dbb-4204-8b3a-49319cc6ca31
sort!(area(shapes))

# ╔═╡ c4773385-4c56-47c0-ab77-827e953047e6
dis = isdisjoint(shapes)

# ╔═╡ dc04542c-1e93-4cde-b659-d9235e86c10e
begin
# Define the radius of the circle
radius = getfield.(shapes, 3)
# Create an array of angles to plot the circle
theta = range(0, 2π, length=100)
# Compute the (x, y) coordinates of the circle points
X = getfield.(shapes, 1) .+ radius .* cos.(theta)
Y = getfield.(shapes, 2) .+ radius .* sin.(theta)
# Plot the circle using the plot function
plot!(X, Y, aspect_ratio=:equal, legend=false)
end

# ╔═╡ 8f7fb0c4-ba18-4025-b322-62c2d17aced8
Area=in(xr,yr,length_points,shapes)/n_points*100*100
